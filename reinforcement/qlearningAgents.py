...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        self.qVals = util.Counter()    #contains q values
        "*** YOUR CODE HERE ***"
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        qVal = self.qVals[(state, action)]
        if qVal == 0:      #unseen state defaults to 0 in Counter()
            return 0.0
        return qVal
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        #self.alpha
        new_q_val = (1 - self.alpha) * self.getQValue(state, action) \
                    + self.alpha * (reward + self.discount * self.computeValueFromQValues(nextState))
        self.qVals[(state, action)] = new_q_val
        #util.raiseNotDefined()
