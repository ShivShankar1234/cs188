class N_gram:
    ...
    def count_words(self):
        ...
        c = collections.Counter(self.text_list)
        # print("aaaaa")
        return c
        ### END YOUR CODE HERE ###
    ...
    def probs_to_neg_log_probs(self, probs_dict):
        ...
        ### START YOUR CODE HERE ###
        # raise NotImplementedError
        neg_log_probs = probs_dict
        neg_log_probs.update((key, -1 * np.log(val)) for key, val in neg_log_probs.items())
    ...
    def filter_adj_counter(self, adj_counter, word_tuple, n):
        ...
        for phrase in adj_counter.keys():
            ...
            if (word_tuple == phrase[:len(word_tuple)]):
    ...
    def p_naive(self, curr_word, prev_phrase, n):
        ...
        try:
            ...
            normalized = self.normalize_dict(filtered_adj_counter)
            prob = 0
            for k, v in normalized.items():
                if (k[len(prev_phrase)] == curr_word):
                    prob = prob + v
    ...
    def calc_neg_log_prob_of_sentence(self, sentence_list, n, p_func=p_laplace):
        ...
        for i in range(0, len(sentence_tuple) - 1):
            ...
            ### START YOUR CODE HERE ###
            # raise NotImplementedError
            cum_neg_log_prob = cum_neg_log_prob + -1 * np.log(curr_word_prob)
    ...
    def calc_prob_of_sentence(self, sentence_list, n, p_func=p_laplace):
        ...
        cum_neg_log = self.calc_neg_log_prob_of_sentence(sentence_list, n)
        return np.exp(-1 * cum_neg_log)
        ### END YOUR CODE HERE ###
