...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        dist = [i[1] for i in self.items()]
        vals = [i[0] for i in self.items()]
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        distribution = self.beliefs

        for pos in self.allPositions:
            obs_prob = self.getObservationProb(observation, gameState.getPacmanPosition(), pos, self.getJailPosition());
            distribution[pos] *= obs_prob
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.allPositions:
            ...
            for newP in newDist.keys():
                dist[newP] += newDist[newP] * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(num):
            self.particles.append(legal_positions[i % len(legal_positions)])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        valid_weight_dist = False
        for weight in weights.values():
            if(weight != 0):
                valid_weight_dist = True
        if not valid_weight_dist:
            self.initializeUniformly(gameState)
        else:
            weights.normalize()
            #remsample from this distribution
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            curr_sample = newPosDist.sample()
            new_samples.append(curr_sample)
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        legal_position_combinations = list(itertools.product(legal_positions, repeat=num_ghosts))
        random.shuffle(legal_position_combinations)

        for i in range(num_parts):
            self.particles.append(legal_position_combinations[i % len(legal_position_combinations)])
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i, particle in enumerate(newParticle):
